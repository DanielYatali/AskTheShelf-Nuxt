...
const items = [
  ...
  {name: 'Basic', svg: 'm21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z'},
  {
    name: 'Link',
...
<template>
  <div class="antialiased">
    ...
    <footer class="z-50 fixed inset-x-0 bottom-0 bg-white text-center p-4">
      <div class="flex items-center py-2 rounded-lg relative">
        ...
        <!--        <div class="relative">-->
        <!--          <button @focusout="isOpen = false" @click="isOpen = !isOpen"-->
        <!--                  class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 focus:outline-none focus:ring flex gap-2 items-center w-28">-->
        <!--            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"-->
        <!--                 stroke="currentColor" class="w-6 h-6">-->
        <!--              <path stroke-linecap="round" stroke-linejoin="round"-->
        <!--                    :d="selectedSearchType.svg"/>-->
        ...
        <!--            {{ selectedSearchType.name }}-->
        <!--          </button>-->
        <!--          <div v-if="isOpen" class="absolute left-0 w-auto mt-1 bg-white shadow-lg rounded z-50 bottom-full">-->
        <!--            <ul class="py-1">-->
        <!--              <li v-for="(item, index) in items" :key="index"-->
        <!--                  class="flex items-center hover:bg-gray-100 cursor-pointer px-4 py-2 gap-2" @click="selectItem(item)">-->
        <!--                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"-->
        <!--                     stroke="currentColor" class="w-6 h-6">-->
        <!--                  <path stroke-linecap="round" stroke-linejoin="round"-->
        <!--                        :d="item.svg"/>-->
