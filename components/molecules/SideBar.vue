...
//   {
//     name: "Michael-Gough",
//     path: "/assistants/Michael-Gough",
//   },
//   {
//     name: "John-Doe",
//     path: "/assistants/John-Doe",
//   },
// ];
...
const activeClass = (path) => {
  return route.path === path ? "bg-primary text-tertiary" : "text-white";
};
...
<template>
  <aside
    class="background-style fixed top-0 left-0 z-40 w-64 h-screen pt-14 transition-transform -translate-x-full md:translate-x-0 overflow-y-auto"
    aria-label="Sidenav"
    :id="identifier"
  >
      ...
      <ul class="space-y-2">
        ...
        <li>
          <button
            ...
            aria-controls="dropdown-assistants"
            data-collapse-toggle="dropdown-assistants"
            ref="assistantDropdown"
          >
            <svg
              aria-hidden="true"
          ...
          <ul id="dropdown-assistants" class="hidden py-2 space-y-2">
            <li v-for="assstant in assistants">
              <a
                @click="navigateTo('/assistants/' + assstant.name)"
                :class="activeClass(assstant.path)"
                class="flex items-center p-2 pl-11 w-full text-base font-medium rounded-lg transition duration-75 group hover:bg-gray-700"
              >
                {{ getAssistantName(assstant["@id"])}}
              </a>
            </li>
          </ul>
        </li>
