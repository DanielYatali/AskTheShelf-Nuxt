    ...
    <ul class="flex overflow-x-auto gap-2 md:gap-6 snap-x snap-mandatory pb-6">
      <template v-for="(object, index) in items" :key="index">
        <li class="shrink-0 snap-center border pb-2 rounded-lg">
          <div class="pt-2">
            <div class="h-20 w-36 mx-auto">
              <img class="object-contain w-full h-full rounded-lg border-round" :src="object.image_url"
                   :alt="object.title">
            </div>
            <div class="grid grid-cols-1 px-2 relative">
              <div class="text-xs line-clamp-3 text-center w-36 h-8">{{ object?.title }}</div>
              <div class="flex justify-between px-2">
                <p class="font-semibold text-xs">${{ object?.price }}</p>
                <div class="font-semibold text-xs flex items-center">{{ object?.rating }}
              ...
              <button @click="buy(object)" class="flex bg-gray-100 rounded-lg items-center mt-1">
                <svg class="h-5 w-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 60" fill-rule="evenodd">
                  <path
  ...
  <template v-else>
    <ul class="flex overflow-x-auto gap-2 md:gap-6 snap-x snap-mandatory pb-6">
      <template v-for="(object, index) in items" :key="index">
        <li class="shrink-0 snap-center border pb-2 rounded-lg">
          <div class="pt-4">
            <div class="h-36 w-72 mx-auto">
              <img class="object-contain w-full h-full rounded-lg border-round" :src="object.image_url"
                   :alt="object.title">
            </div>
            <div class="grid grid-cols-1 px-2 relative">
              <div class="mb-3 mt-3 font-medium line-clamp-2 text-center max-w-72 h-12">{{ object?.title }}</div>
              <div class="flex justify-between px-2">
              ...
              <!--                Details-->
              <!--              </button>-->
              <button @click="buy(object)" class="flex bg-gray-100 rounded-lg items-center mt-2">
